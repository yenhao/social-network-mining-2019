...
def get_trace(X_pca, data, category, color):
    ...
    trace = go.Scatter3d(
        x=X_pca[data[data.apply(lambda x: True if x==category else False)].index, 0],
        y=X_pca[data[data.apply(lambda x: True if x==category else False)].index,1],
        z=X_pca[data[data.apply(lambda x: True if x==category else False)].index,2],
        mode='markers',
        marker=dict(
            size=4,
            line=dict(
        ...
        text=data[data.apply(lambda x: True if x==category else False).index].tolist()
    )
    return trace
